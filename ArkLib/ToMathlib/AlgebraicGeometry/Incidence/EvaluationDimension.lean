...
/-- The affine evaluation equation for a degree-`< k` coefficient vector and one challenge
coordinate.  At a point `(z, coeff)`, it reads `∑ l, coeff_l * α^l = f + z * g`. -/
def affineCoefficientEvaluation (k : ℕ) (α f g : F) :
    MvPolynomial (Option (Fin k)) F :=
  (∑ l : Fin k, C (α ^ l.val) * X (some l)) - (C f + X none * C g)
...
theorem coefficientEvaluation_hilbertPolynomial_natDegree_le
  ...
  have hfull (i : Fin c) :
      (∑ l : Fin k, algebraMap F A ((α i) ^ l.val) * coeff l) =
        algebraMap F A (f i) + z * algebraMap F A (g i) := by
    have hz : Ideal.Quotient.mk J
        (affineCoefficientEvaluation k (α i) (f i) (g i)) = 0 :=
    ...
    have hC (a : F) : Ideal.Quotient.mk J (C a) = algebraMap F A a := by
      ...
      rfl
    simpa only [affineCoefficientEvaluation, map_sub, map_sum, map_mul,
      map_add, hC, sub_eq_zero, z, coeff] using hz
  have hlow : ∀ i, ∑ l : Fin c, algebraMap F A ((α i) ^ l.val) *
    ...
    have hrhs : algebraMap F A (f i) + z * algebraMap F A (g i) ∈ B := by
      apply B.add_mem (B.algebraMap_mem _)
      apply B.mul_mem
      · exact Algebra.subset_adjoin (Set.mem_insert z _)
...
theorem retainedPrime_hilbertPolynomial_natDegree_le_of_coefficientLocalization
    [Finite σ] {τ : Type*} [Finite τ] {k c : ℕ}
    {J : Ideal (MvPolynomial τ F)} (hJ : J.IsPrime)
    {t : MvPolynomial τ F} (ht : t ∉ J)
    (hJdim : (hilbertPolynomial J).natDegree ≤ k + 1 - c)
