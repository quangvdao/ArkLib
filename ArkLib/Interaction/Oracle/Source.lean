...
def handler (S : SourceCtx.{u, w, v} I E) (env : E) : QueryImpl S.spec Id :=
  fun q => S.impl q env
...
def empty : SourceCtx.{u, w, v} PEmpty PUnit where
...
def reindex (S : SourceCtx.{u, w, v} I E) (f : J → I) : SourceCtx J E where
  spec := fun q => S.spec (f q)
  impl := fun q => S.impl (f q)

/-- Change only the backing-data presentation. -/
def comapEnv (S : SourceCtx.{u, w, v} I E) (f : F → E) : SourceCtx I F where
  spec := S.spec
...
def tensor (S : SourceCtx.{u, w, v} I E) (T : SourceCtx.{u', w', v} J F) :
...
def parallel (S : SourceCtx.{u, w, v} I E) (T : SourceCtx.{u', w', v'} J F) :
    SourceCtx (I × J) (E × F) where
...
def family {A : Type k} {Q : A → Type u} {Env : A → Type w}
    (S : (a : A) → SourceCtx.{u, w, v} (Q a) (Env a)) :
...
def liftResponse (S : SourceCtx.{u, w, v} I E) : SourceCtx.{u, w, max v v'} I E where
  spec := fun q => ULift.{v'} (S.spec q)
  impl := fun q env => ⟨S.handler env q⟩

@[simp]
theorem tensor_handler_inl (S : SourceCtx.{u, w, v} I E)
    (T : SourceCtx.{u', w', v} J F) (env : E × F) (q : I) :
    (S.tensor T).handler env (.inl q) = S.handler env.1 q := rfl

@[simp]
theorem tensor_handler_inr (S : SourceCtx.{u, w, v} I E)
    (T : SourceCtx.{u', w', v} J F) (env : E × F) (q : J) :
    (S.tensor T).handler env (.inr q) = T.handler env.2 q := rfl

@[simp]
theorem parallel_handler (S : SourceCtx.{u, w, v} I E)
    (T : SourceCtx.{u', w', v'} J F) (env : E × F) (q : I × J) :
...
def eval (S : SourceCtx.{u, w, v} I E) (env : E) {α : Type v}
...
theorem eval_eq_of_handler_eq (S : SourceCtx.{u, w, v} I E) {env env' : E}
...
theorem handler_eq_iff_eval_eq (S : SourceCtx.{u, w, v} I E) (env env' : E) :
...
structure SourceHom (S : SourceCtx.{u, w, v} I E) (T : SourceCtx.{u', w', v'} J F) where
...
variable {S : SourceCtx.{u, w, v} I E} {T : SourceCtx.{u', w', v'} J F}
  {U : SourceCtx.{u'', w'', v''} K G} {V : SourceCtx.{u''', w''', v'''} L H}
...
def id (S : SourceCtx.{u, w, v} I E) : SourceHom S S where
...
def fromReindex (S : SourceCtx.{u, w, v} I E) (f : J → I) :
...
def toComapEnv (S : SourceCtx.{u, w, v} I E) (f : F → E) :
...
def inl (S : SourceCtx.{u, w, v} I E) (T : SourceCtx.{u', w', v} J F) :
...
def inr (S : SourceCtx.{u, w, v} I E) (T : SourceCtx.{u', w', v} J F) :
...
def familyMap {A : Type k} {B : Type k'}
    ...
    (S : (a : A) → SourceCtx.{u, w, v} (QA a) (EA a))
    (T : (b : B) → SourceCtx.{u', w', v'} (QB b) (EB b))
...
def inFamily {A : Type k} {Q : A → Type u} {Env : A → Type w}
    (S : (a : A) → SourceCtx.{u, w, v} (Q a) (Env a)) (a : A) :
...
def toLiftResponse (S : SourceCtx.{u, w, v} I E) :
    SourceHom S (S.liftResponse.{u, w, v, v'}) where
  route := ⟨fun q => q, fun _ answer => answer.down⟩
  onEnv := fun env => env
  commutes := fun _ _ => rfl

/-- Route out of explicitly lifted responses and rewrap the answer. -/
def fromLiftResponse (S : SourceCtx.{u, w, v} I E) :
    SourceHom (S.liftResponse.{u, w, v, v'}) S where
...
variable {A : SourceCtx.{u, w, v} I E} {B : SourceCtx.{u', w', v} J F}
...
structure SourceEquiv (S : SourceCtx.{u, w, v} I E) (T : SourceCtx.{u', w', v'} J F) where
...
def refl (S : SourceCtx.{u, w, v} I E) : SourceEquiv S S :=
  ⟨SourceHom.id S, SourceHom.id S, rfl, rfl⟩

/-- Reverse an equivalence. -/
def symm {S : SourceCtx.{u, w, v} I E} {T : SourceCtx.{u', w', v'} J F}
...
def trans {S : SourceCtx.{u, w, v} I E} {T : SourceCtx.{u', w', v'} J F}
    {U : SourceCtx.{u'', w'', v''} K G} (e : SourceEquiv S T) (f : SourceEquiv T U) :
...
def liftResponse (S : SourceCtx.{u, w, v} I E) :
    SourceEquiv S (S.liftResponse.{u, w, v, v'}) where
