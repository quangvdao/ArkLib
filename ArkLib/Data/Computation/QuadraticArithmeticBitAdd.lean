...
/-!
# A literal alias-safe scalar-register ADD instruction
...
aliasing either source. The arithmetic result is copied into the destination, then the temporary
result is explicitly cleared. All copies and clearing use actual local-bit successors.
...
This module lowers one ADD instruction; it does not initialize registers or compile a full program.
-/
...
inductive Control where
  ...
  | adding (dst : Register) (r : Registers) (child : PaddedModAdd.Control)
...
def step : Control → Option Control
  ...
  | .right q y dst r left child =>
      ...
      | none => match child with
          | .done source right =>
              some (.adding dst (Function.update r y source) (.adding (.start q left right)))
          | _ => none
  | .adding dst r child =>
      match PaddedModAdd.step child with
      | some next => some (.adding dst r next)
      | none => match child with
...
def registers : Control → Registers
  | .start _ _ _ _ r | .adding _ r _ | .clearing _ r _ | .done _ r => r
