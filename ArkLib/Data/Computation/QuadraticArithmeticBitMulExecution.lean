...
/-!
# Actual MUL instruction refinement with all register aliases

The same physical execution realizes the existing arithmetic machine's MUL clause. Its bound
...
theorem lift_mul (dst : Register) (r : Registers) {n : ℕ}
    {s t : PaddedMul.Control} (h : PaddedMul.Trace n s t) :
    Trace n (.multiplying dst r s) (.multiplying dst r t) := by
  induction h with
  | nil => exact .nil _
  | cons head _ ih => exact .cons (by simp only [step, head]) ih
...
theorem load_trace (q : Word) (x y dst : Register) (r : Registers) :
    Trace (2 * (r x).length + 2 * (r y).length + 9) (.start q x y dst r)
      (.multiplying dst r (.normalizing q (r y) (.startAdd (r x) [] false))) := by
  ...
  have ho : Trace 1 (.right q y dst (Function.update r y []) (r x) (.done (r y) (r y)))
      (.multiplying dst r (.normalizing q (r y) (.startAdd (r x) [] false))) :=
    .cons (by simp [step, WordCopyMachine.step]) (.nil _)
...
/-- The retained multiplier operand is explicitly cleared before result write-back. -/
theorem clearRight_trace (q : Word) (dst : Register) (r : Registers) (out right : Word) :
    Trace (right.length + 1) (.clearRight q dst r out right)
...
theorem store_trace (q : Word) (dst : Register) (r : Registers) (out right : Word) :
    Trace (right.length + (r dst).length + 3 * out.length + 7)
      (.multiplying dst r (.padding right (.padding q (.done out))))
      (.done q (Function.update r dst out)) := by
  have hi : Trace 1 (.multiplying dst r (.padding right (.padding q (.done out))))
      (.clearRight q dst r out right) := .cons rfl (.nil _)
  have hr := clearRight_trace q dst r out right
...
theorem mul_execution (q : Word) (x y dst : Register) (r : Registers)
    (hwidth : ∀ i, (r i).length = q.length) (hred : ∀ i, value (r i) < value q) :
    ∃ n ≤ value q * (24 * q.length + 48) + 15 * q.length + 32, ∃ out,
  ...
  obtain ⟨na, hna, out, ha, _hra, hw, hv, hb⟩ :=
    PaddedMul.multiply_fixed_width q (r x) (r y) (hred x) (hred y) (hwidth x) (hwidth y)
  have hs := store_trace q dst r out (r y)
  have hall := (hl.append (lift_mul dst r ha)).append hs
...
/-- One bit run refines the MUL successor and retains the other physical state. -/
theorem source_execution (mem : AddressedBits.Memory) (q : Word)
    ...
    ∃ n ≤ value q * (24 * q.length + 48) + 15 * q.length + 32, ∃ out,
      ...
      QuadraticAlgebra.ArithmeticMachine.step (decodeInput q input)
        (.running (.mul x y dst :: rest) (decodeRegisters q r) flags) =
        some (.running rest (decodeRegisters q (Function.update r dst out)) flags,
          QuadraticAlgebra.ArithmeticMachine.instructionCost (.mul x y dst)) := by
  obtain ⟨n, hn, out, ht, hf, hw, hb, hd, _hw, _hb⟩ := mul_execution q x y dst r hwidth hred
