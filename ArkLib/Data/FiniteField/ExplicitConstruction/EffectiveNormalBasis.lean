...
/-!
# Supplied normal-coordinate operational adapter
...
/-- Operational data furnished by a supplied absolute normal basis. Coordinates
are ordered by successive Frobenius powers of the basis generator. -/
structure SuppliedNormalCoordinates (p d : Nat) (K : Type*) [Field K] where
  prime : p.Prime
...
theorem inverseFrobenius_pow (N : SuppliedNormalCoordinates p d K) (a : K) :
...
/-- Prime-field radix digits, interpreted in the supplied normal coordinates. -/
def elementEquiv (N : SuppliedNormalCoordinates p d K) : Fin (p ^ d) ≃ K :=
...
def effectiveField [BEq K] [LawfulBEq K] (N : SuppliedNormalCoordinates p d K) :
