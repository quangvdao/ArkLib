...
theorem prescribed_symbolic_band_certificate_inputs
    ...
    (hblock : 8 * Nat.ceil
      (100 * (Nat.ceil (Real.exp ((169 / 25) / δ)) : ℝ) ^ 2 *
        ReedSolomon.AllRateListDecoding.harmonicNumber
          (Nat.ceil (Real.exp ((169 / 25) / δ)) - 1)) ≤ n)
    (hA : ReedSolomon.AllRateListDecoding.agreementThreshold δ n k ≤ n) :
    let A := ReedSolomon.AllRateListDecoding.agreementThreshold δ n k
    let d := Nat.ceil (Real.exp ((169 / 25) / δ))
    let H := ReedSolomon.AllRateListDecoding.harmonicNumber (d - 1)
    let m := Nat.ceil (100 * (d : ℝ) ^ 2 * H)
    let D := ReedSolomon.AllRateListDecoding.asymmetricBandAmbientDimension δ n k - 1
    let g₀ := min 1 (δ / ((D : ℝ) / n))
  ...
  obtain ⟨_hsize, _hD₀, hdD, hρlo, hρhi⟩ :=
    ReedSolomon.AllRateListDecoding.band_block_size_bounds δ n k hδ hδ' hk hblock hA
  have hρ : 0 < (D : ℝ) / n := lt_of_lt_of_le (by positivity) hρlo
  obtain ⟨_hd, _hH, _hHd, _hδH, _hg, hg₁, _hag, _hgm, hm, _hW, _hmass⟩ :=
    ReedSolomon.AllRateListDecoding.band_rate_parameter_estimates
  ...
  have hAreal : (k : ℝ) + δ * n ≤ A :=
    (ReedSolomon.AllRateListDecoding.agreementThreshold_le_iff_real
      hδ.le n k A).mp le_rfl
  have hslack : (D : ℝ) * (1 + g₀) ≤ A := by
    have hs := ReedSolomon.AllRateListDecoding.asymmetricBandAmbientDegree_slack_le_agreement
      hδ hk hD hAreal
    change (D : ℝ) * (1 + min 1 (δ * n / D)) ≤ A at hs
    rw [← ReedSolomon.AllRateListDecoding.band_relativeSlack_rate_eq hn hD] at hs
    exact hs
  ...
  have hbudget : 0 < m * A := by
    have hApos : 0 < A := by
      dsimp only [A, ReedSolomon.AllRateListDecoding.agreementThreshold]
      omega
    exact Nat.mul_pos hm_pos hApos
  have hkD : k ≤ D + 1 := by
    dsimp only [D]
    rw [Nat.sub_add_cancel (by
      have := ReedSolomon.AllRateListDecoding.asymmetricBandAmbientDimension_pos
        (delta := δ) (n := n) hk
      omega)]
