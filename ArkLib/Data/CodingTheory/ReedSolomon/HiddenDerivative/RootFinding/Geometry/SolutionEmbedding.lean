...
theorem exists_regular_solution_jet_family
  ...
  · intro jet hjet
    ...
    · intro l hl
      exact polynomialJet_mem_highTaylorCuts center QE K k P hp.1 hp.2.1 hs hbinE _
        (Ideal.subset_span ⟨⟨l, hl⟩, rfl⟩)
    · have heq : (Finset.univ.filter fun i ↦
          aeval (polynomialJet center P)
            (taylorAgreementEquation center QE K (f (domain i)) (f (received i))) = 0) =
          (Finset.univ.filter fun i ↦ P.eval (f (domain i)) = f (received i)) := by
        ext i
        simp only [Finset.mem_filter, Finset.mem_univ, true_and]
        exact polynomialJet_agreement_cut_iff center QE K P
          (hp.1.trans_le (Nat.cast_le.mpr hkK)) hp.2.1 hs hbinE _ _
      rw [heq]
