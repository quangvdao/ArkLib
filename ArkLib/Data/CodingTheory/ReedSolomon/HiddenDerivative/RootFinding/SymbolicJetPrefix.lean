...
theorem Presentation.jetDegree {Q : DifferentialPolynomial R d} {s : Fin (d + 1)}
    ...
    HiddenDerivative.jetDegree A.equation j =
      HiddenDerivative.jetDegree Q
...
theorem Presentation.top_jetDegree {Q : DifferentialPolynomial R d} {s : Fin (d + 1)}
    (A : Presentation Q s) :
    HiddenDerivative.jetDegree A.equation (Fin.last s.val) =
      HiddenDerivative.jetDegree Q s := by
...
theorem Presentation.highest {Q : DifferentialPolynomial R d} {s : Fin (d + 1)}
  ...
  constructor
  · change 0 < HiddenDerivative.jetDegree A.equation (Fin.last s.val)
    rw [A.top_jetDegree]
...
theorem Presentation.separant {Q : DifferentialPolynomial R d} {s : Fin (d + 1)}
    ...
    MvPolynomial.rename (jetPrefixEmbedding s)
        (HiddenDerivative.separant A.equation (Fin.last s.val)) =
      HiddenDerivative.separant Q s := by
  rcases A with ⟨equation, he⟩
  dsimp only
  rw [← he, HiddenDerivative.separant, HiddenDerivative.separant,
...
theorem Presentation.jetEvaluation {E : Type*} [Field E] (φ : R →+* E)
    ...
    HiddenDerivative.jetEvaluation (MvPolynomial.map φ A.equation) center (restrictJet s jet) =
      HiddenDerivative.jetEvaluation (MvPolynomial.map φ Q) center jet := by
  rcases A with ⟨equation, he⟩
  dsimp only
...
theorem Presentation.separant_specialization {E : Type*} [Field E] (φ : R →+* E)
    ...
    differentialSpecialization
        (HiddenDerivative.separant (MvPolynomial.map φ A.equation) (Fin.last s.val)) P =
      differentialSpecialization (HiddenDerivative.separant (MvPolynomial.map φ Q) s) P := by
  rcases A with ⟨equation, he⟩
  dsimp only
...
theorem exists_stage_presentation {F : Type*} [Field F]
    ...
    ∃ A : Presentation stage.1 stage.2,
      ...
      HiddenDerivative.jetDegree A.equation (Fin.last stage.2.val) ≤
        HiddenDerivative.jetDegree Q stage.2 ∧
      ∀ u, (MvPolynomial.coeff u A.equation).natDegree ≤ h := by
