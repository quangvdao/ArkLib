...
theorem Presentation.separant {ts : List (Term F)} {Q : DifferentialPolynomial F d}
    ...
    rename (jetPrefixEmbedding s) (HiddenDerivative.separant (semanticEquation A.polynomial)
      (Fin.last s.val)) = HiddenDerivative.separant Q s := by
  rcases A with ⟨poly, _, he⟩
  dsimp only
...
theorem Presentation.jetDegree {ts : List (Term F)} {Q : DifferentialPolynomial F d}
    ...
    HiddenDerivative.jetDegree (semanticEquation A.polynomial) j =
      HiddenDerivative.jetDegree Q ⟨j.val, lt_of_le_of_lt (Nat.le_of_lt_succ j.isLt) s.isLt⟩ := by
  rcases A with ⟨poly, _, he⟩
  dsimp only
  rw [← he]
...
theorem Presentation.characteristic {ts : List (Term F)} {Q : DifferentialPolynomial F d}
    {s : Fin (d + 1)} (A : Presentation ts Q s)
    (hchar : ∀ j, HiddenDerivative.jetDegree Q j < ringChar F) :
    ∀ j, HiddenDerivative.jetDegree (semanticEquation A.polynomial) j < ringChar F := by
  intro j
  rw [A.jetDegree]
  exact hchar _
...
theorem Presentation.top_degree {ts : List (Term F)} {Q : DifferentialPolynomial F d}
    {s : Fin (d + 1)} (A : Presentation ts Q s) :
    HiddenDerivative.jetDegree (semanticEquation A.polynomial) (Fin.last s.val) =
      HiddenDerivative.jetDegree Q s := A.jetDegree (Fin.last s.val)
...
theorem active_weight_le (D : ℕ) (Q : DifferentialPolynomial F r) (j : Fin (r + 1))
  ...
  have hex : ∃ u ∈ Q.support, 0 < u (some j) := by
    by_contra! hn
    have hz : HiddenDerivative.jetDegree Q j ≤ 0 := degreeOf_le_iff.mpr hn
...
theorem of_chain_record {ts : List (Term F)} {Q : DifferentialPolynomial F d}
    ...
    ∃ R : DifferentialPolynomial F d, ∃ s : Fin (d + 1), ∃ A : Presentation record.equation R s,
      i = s.val + 1 ∧
      e = HiddenDerivative.jetDegree (semanticEquation A.polynomial) (Fin.last s.val) ∧
