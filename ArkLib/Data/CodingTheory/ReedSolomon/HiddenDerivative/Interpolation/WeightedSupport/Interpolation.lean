...
is the lower-cutoff weighted support itself, so the resulting polynomial retains all its
...
def weightedSupportGlobalConstraint (hD : 0 < D) (centers received : ι → F) :
    weightedSupportSpace F D d W Cmin L hD →ₗ[F] (ι → LocalPolynomial F d) :=
    ...
    _ ≤ ∑ _i : ι, localCoordinateBudget d m W ⌈L / D - Cmin⌉₊ :=
      Finset.sum_le_sum fun i _ ↦
        finrank_weightedSupportLocalConstraint_le (d := d) (m := m) (W := W)
          (Cmin := Cmin) (L := L) hd hD (centers i) (received i)
    _ = _ := by simp
...
theorem exists_nonzero_weightedSupport_interpolant (hd : 0 < d) (hD : 0 < D)
    ...
    (hdim : Fintype.card ι * localCoordinateBudget d m W ⌈L / D - Cmin⌉₊ <
      (weightedSupportExponents D d W Cmin L hD).card) :
