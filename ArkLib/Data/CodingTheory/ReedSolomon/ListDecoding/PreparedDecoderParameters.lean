...
theorem prescribed_search {q : ℕ} [Fact q.Prime] (delta : ℝ) (hdelta : 0 < delta)
    ...
    ∃ found c, AmbientSearchMachine.run k d m A rows = (some found, c) ∧
      ...
      asymmetricBandAmbientDimension delta n k - 1 ≤ found.degree ∧ found.degree < n ∧
  ...
  obtain ⟨found, c, hr, hl, hu, hc, hcost⟩ := AmbientSearchMachine.run_prescribed_band delta
    hdelta hquarter n k A hblock hk hkn hA hAn domain received
  ...
  exact ⟨found, c, hr, hc, hl, hu, hchar, hweight, sample_count_le_square hblock hAn hnq,
    field_ne_two (asymmetricBandMultiplicity_pos hdelta hquarter) hblock hnq, hcost⟩
